...
const PinkCalendarView = () => {
  ...
  const dateCellRender = value => {
    ...
    return (
      <ul>
        {data.map(item => <li key={item._id}>{item.slotGuest}{item.displayDate}<Label as='a' color='pink' tag>Booked</Label></li>)}
      </ul>
    )
  } 
...
export default PinkCalendarView;
