...
const AppointmentForm = (props) => {
  ...
  useEffect(() => {
    // SetBooking({slotDate:  location.state.slotDate, exercise: location.state.exercise, food: location.state.food});
    setInput({...input, slotDate: moment(location.state.slotDate, 'MM-DD-YYYY')});
  }, [location]);
  ...
  const submitHandler = event => {
    ...
    if (validateInput()) {
      ...
      const paperStyle={padding :20,height:'75vh',width:600, margin:"20px auto", backgroundColor:'#fff5cc'}
  return (
    <div>
    ...
    <Form className='appt-form' onSubmit={submitHandler}>
      ...
      <DatePicker disabled value={moment(input.slotDate, 'MM-DD-YYYY')} onChange={dateHandler}/>
      {error && <div>{error}</div>}
