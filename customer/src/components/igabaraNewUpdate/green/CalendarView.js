...
const GreenCalendarView = () => {
  ...
  const dateCellRender = value => {
    ...
    return (
      <ul>
        {data.map(item => <li key={item._id}>{item.displayDate}<Label as='a' color='green' tag>unavailable</Label></li>)}
      </ul>
    )
  } 
...
export default GreenCalendarView;
